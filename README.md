...
- **新着動画レコメンド** - 登録ポッドキャストのembedding（Gemini embeddingモデル + pgvector）から興味プロファイルを生成し、登録したYouTubeチャンネルの新着動画を日次でスコアリング。類似度が閾値を超えた動画をLINEで通知。通知の「PodQueueに登録」ボタンからワンタップで聞きたいリストに追加可能
...
# レコメンドcron（Vercel Cronの認証用シークレット）
